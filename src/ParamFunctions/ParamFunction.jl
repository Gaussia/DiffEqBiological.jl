...
function ode_def_opts(name::Symbol,opts::Dict{Symbol,Bool},ex::Expr,params...;M=I,depvar=:t)
  ...
  fex = deepcopy(ex) # Save this expression as the expression for the call
  push!(fex.args, nothing)
  pex = deepcoyp(ex)
...
#typeex,constructorex = make_parameterized_type(name,params,origex,funcs,syms,fex,pex=pex,
...
very much a copy-paste from ParameterizedFunctions.jl.

Intended to generate a ParameterizedFunctions as the ode part of the Reaction.

input:
- funcs::Vector{Expr}
    the right-hand-side of our ODE.
- params::Vector{Symbol}
- indvar_dict::OrderedDict{Symbol, Integer}
- opts::Dict
    specify which symbolic calculations to perform.

"""


function generate_parameterized_function(name::Symbol, funcs, ex, params, indvar_dict, opts::Dict{Symbol, Bool}, origex; depvar=:t)
  name = Symbol(:Parameterized, name)
  syms = collect(keys(indvar_dict))
